      ...
      <ScrollView className="flex-1" contentContainerClassName="px-6 py-4">
        ...
        {searchResult && !isLoading && !isShowingAd && (
          <View className="mt-4">
            ...
            {/* Display web results for competitive layer, otherwise show answer text */}
            {searchResult.source === "competitive" &&
            (
            ...
            (
              ...
              <View className="mb-4">
                {(() => {
                  ...
                  return (
                    <>
                      <Text className="text-gray-700 text-base font-outfit-semi-bold mb-4">
                        Top {webResults.length} Search Results:
                      </Text>
                      {webResults.map(
                        (
                          result: {
                          ...
                          <TouchableOpacity
                            key={index}
                            className="bg-white rounded-2xl p-5 mb-4 border-2 border-gray-200 shadow-sm"
                            activeOpacity={0.7}
                            onPress={async () => {
                              // Open URL in browser
                              try {
                                const WebBrowser = await import(
                                  "expo-web-browser"
                                );
                          ...
                          >
                            {/* Result Number Badge - Makes it clear these are separate */}
                            <View className="flex-row items-center mb-3">
                              ...
                              <View className="flex-1">
                                <Text className="text-gray-500 text-xs font-outfit-regular">
                                  Result {index + 1} of {webResults.length}
                                </Text>
                              </View>
                              <Ionicons
                                name="open-outline"
                            ...
                            {/* Title */}
                            <Text className="text-blue-600 text-lg font-outfit-semi-bold mb-2 leading-6">
                              {result.title}
                            </Text>

                            {/* URL */}
                            <Text className="text-green-600 text-xs font-outfit-regular mb-3">
                              {result.url}
                            </Text>

                            {/* Snippet/Summary - More prominent with background */}
                            {result.snippet &&
                            result.snippet.trim().length > 0 ? (
                              <View className="bg-gray-50 rounded-xl p-4 mt-2 border border-gray-100">
                                <Text className="text-gray-700 text-xs font-outfit-semi-bold mb-2 uppercase tracking-wide">
                                  Summary:
                                </Text>
                                <Text className="text-gray-900 text-base font-outfit-regular leading-6">
                                  {result.snippet}
                                </Text>
                              </View>
                            ) : (
                              <View className="bg-gray-50 rounded-xl p-4 mt-2 border border-gray-100">
                                <Text className="text-gray-400 text-sm font-outfit-regular italic">
